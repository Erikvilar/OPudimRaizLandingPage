const products = {
  products: [
    {
      img: "src/images/pudimTradicionalSobre.jpeg",
      text: "Pudim Raiz, a sobremesa perfeita para seu dia. Pudim tradicional tamanho em familia 1kg, serve de 6 a 8 pessoas, um pduim com cremosidade que remete o sabor da sua infancia.",
      name: "Pudim Tradicional",
      button:"Eu quero!"
    },
    {
      img: "src/images/pudim1.jpg",
      text: "produzido com chocolate nobre e a calda de morangos frescois, feito artesanalmente. Um pudim que vai além da beleza, com o sabor marcante da combinação de sabores. Pudim tamanho familia 1kg, serve de 6 a 8 pessoas. ",
      name: "Pudim de chocolate branco com calda de morango",
      button:"Eu quero!"
    },
    {
      img: "src/images/pudim2.jpg",
      text: "O pudim de café é uma sobremesa sofisticada que combina a suavidade e cremosidade de um pudim clássico com o sabor profundo e marcante do café. A base é feita com uma mistura tradicional de leite condensado, leite e ovos, mas enriquecida com café forte, que confere à sobremesa uma coloração marrom-aveludada e um aroma irresistível.",
      name: "Pudim de Café",
      button:"Eu quero!"
    },
    {
      img: "src/images/pudim4.jpg",
      text: "Copo da Felicidade montado com, creme de Ninho feito com chocolate branco, brigadeiro de chocolate meio amargo e morangos frescos. Nossos recheios não tem adição de trigo nem qualquer tipoo de amido.",
      name: "Copo da felicidade 250ml",
      button:"Eu quero!"
    },
   
  ],
  Background: [
    {
      img: "https://recipesblob.oetker.com.br/assets/448ca0c7941945199bd2136f3909b09c/1272x764/bolo-pudim-de-chocolate.webp",
      text:"Trabalhamos com os melhores ingredientes e de mais alta qualidade trazendo o melhor para nossos clientes.",
      name:"Experiêncie o melhor do sabor tradicional O Pudim Raiz",
      button:"Veja mais"
    },
    {
      img: "https://img.cybercook.com.br/receitas/723/pudim-de-leite-condensado-sem-ir-ao-fogo.jpeg",
       text:"Um pouco da confeitaria mineira especializada em pudim diretamente para voçê.",
       name:"Experiêncie o melhor do sabor tradicional o pudim raiz",
       button:"Veja mais"
    },   
    {
      img: "https://www.mococa.com.br/wp-content/uploads/2023/08/img-pudim.jpg",
       text:"De fã para fã fazemos os melhores pudins da região venha e experiemente!.",
       name:"Experiêncie o melhor do sabor tradicional o pudim raiz",
       button:"Veja mais"
    }
   

  ],
};
export default products;
